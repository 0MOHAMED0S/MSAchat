    ...
    <div class="container">
        ...
        <div class="d-flex align-items-center mb-3 p-2 rounded shadow-sm bg-white">
            <img src="{{ $receiver->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode($receiver->name) . '&background=random' }}"
                alt="{{ $receiver->name }}" class="rounded-circle me-3" style="width:50px; height:50px; object-fit:cover;">
            <div>
    ...
    `;
        ...
        function subscribeToChat(convoId) {
            ...
            if (currentChatSubscription && Number(currentChatSubscription) !== Number(convoId)) {
                try {
                    Echo.leave(`chat.${currentChatSubscription}`);
                } catch {}
            }
        ...
        Echo.join('online-users')
            ...
            .joining(user => {
                if (Number(user.id) === partnerId) statusEl.style.background = 'green';
            })
            .leaving(user => {
                if (Number(user.id) === partnerId) statusEl.style.background = 'gray';
            });
