...
<script type="module">
    ...
    // --- Last message, timestamp & unread counter ---
    ...
    // --- Reset badge when conversation is read ---
    Echo.private(`chat-list.${myId}`)
        .listen("MessageRead", (e) => {
            if (e.readerId === myId) {
                let badge = document.getElementById(`unread-badge-${e.conversation_id}`);
                let lastMsg = document.getElementById(`last-message-${e.conversation_id}`);
                if (badge) {
                    badge.textContent = 0;
